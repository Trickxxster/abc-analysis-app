...
# Настройка страницы
# ------------------------------------------------------------
st.set_page_config(page_title="ABC-анализ и расчёт заказа", layout="wide")
st.title("📊 Автоматизация расчёта заказа и ABC-анализ")

# ------------------------------------------------------------
# Города, которые ожидаются в файле
# ------------------------------------------------------------
EXPECTED_CITIES = [
...
# ------------------------------------------------------------
# Вспомогательная функция: привести столбец к числам
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Функция поиска колонок для города
# ------------------------------------------------------------
def find_city_columns(df: pd.DataFrame, city: str):
    ...
    Возвращает словарь с названиями найденных колонок или None, если что-то не найдено.
    """
    ...
    for key, word in keywords.items():
        candidates = [
            ...
            if city_lower in col.lower() and word in col.lower()
        ]
        if candidates:
...
if uploaded_file is not None:
    ...
    for city in EXPECTED_CITIES:
        ...
        city_df["Потребность (сырая)"] = need.round(2)  # для отладки, не обязательно
    ...
    def generate_excel(matrix, city_data):
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            # Листы по городам
            for city, cdf in city_data.items():
                # Возвращаем 'Товар' как колонку
                city_sheet = cdf.reset_index()
                # Убираем лишние столбцы, если есть
                cols_to_save = [
    ...
    with tab2:
        ...
        if selected_city:
            ...
            # Покажем только значимые колонки
            display_cols = [
                "Товар", "Остаток", "Продажи", "В пути",
                "Среднедневные продажи", "К заказу", "Категория ABC"
            ]
